...
NEG_GROUPS = [
 ...
 ("영양제 확장 누수 (9/26 추가, 두 캠페인)",
  ["10대","10 대","남성","직장인","피곤","탈모","모발","피부","유산균","오메가",
   "종합","에스더","코스트코","프라젠","활기마이트","자보티바","파비플로라",
   "알래스칸","레시틴","vitamin","vitamins","supplement","supplements",
   "pharmacy","energy"], True),
...
GA4 = [("2026-09-09",1007,19,774700),("2026-09-10",1313,13,585300),("2026-09-11",1178,12,505340),
 ...
 ("2026-09-23",1012,8,825800),("2026-09-24",1130,14,1321242),
 ("2026-09-25",1328,6,828100),("2026-09-26",304,0,0)]
BASE_N = 12
DAILY = [   # (날짜, 캠페인, 노출, 클릭, 비용, 전환, 전환값)
 ...
 ("2026-09-25", "붓기",       827,150, 18823, 0, 0),
 ("2026-09-25", "칼륨",       463,193, 19848, 0, 0),
 ("2026-09-25", "브랜드 방어",  18, 2,   778, 0, 0),
]
...
TODO = [
 ("위험", "검색 파트너 네트워크 — 사흘째, 누적 59,917원",
  "9/24 11,998원 · 9/25 <b>32,534원</b> · 9/26 오전까지 15,385원. 전부 <b>검색 파트너</b>"
  "(구글 검색이 아닌 제휴 사이트)에서 나갔고 전환은 0입니다. CTR 26~44%(정상 1.6%), "
  "CPC 84~293원, 100% 모바일. 구글 무효클릭 필터는 3.5%만 걸러냅니다. "
  "<b>구글애드 UI → 캠페인 선택 → 설정 → 네트워크 → 「Google 검색 파트너 포함」 체크 해제.</b> "
  "붓기·칼륨 두 캠페인 모두. API에 이 설정이 없어 제가 못 합니다."),
 ("위험", "제외 키워드로는 못 막는다 — 오늘 증명됨",
  "9/25에 「식품」 계열 19개를 넣자 그 검색어들은 9/26에 사라졌습니다. 대신 "
  "<b>「10대 영양제」 82클릭 7,720원</b>, 「영양제 추천」, 「남성 영양제」, 「여에스더 유산균」, "
  "「코스트코 영양제 추천」이 그 자리를 채웠습니다. 뿌리가 <b>「칼륨 영양제」·「붓기 영양제」 "
  "확장검색</b>이라 「영양제」가 들어간 모든 검색으로 번집니다. 그런데 「영양제」는 우리 "
  "키워드라 제외할 수 없습니다. <b>확장검색 3개 일시중지</b>가 유일한 해법입니다. "
  "말씀만 주시면 1분입니다 — 대표님이 직접 고르신 키워드라 제 판단으로 끄지 않았습니다."),
...
SETUP = [
 ...
 "캠페인 3 · 광고그룹 3 · 광고 3 · 키워드 42 · 제외 키워드 146+ · 광고 확장 30 · 위치 · 언어 · 입찰을 모두 API로 설정했습니다.",
...
w('<div class="note" style="border-left-color:var(--crit)">'
  ...
  '<p><b>사흘째입니다. 검색 파트너 누수 누적 59,917원.</b> '
  '9/25 하루에만 파트너 사이트에서 338클릭 · <b>32,534원</b>이 나갔습니다. '
  '그날 전체 광고비 39,449원의 82%입니다. 전환 0.</p>'
  '<p style="margin-top:8px"><b>제외 키워드로는 못 막는다는 것이 오늘 증명됐습니다.</b> '
  '9/25에 넣은 19개로 「건강보조식품」 계열은 오늘 사라졌지만, 바로 '
  '「10대 영양제」(82클릭 7,720원) 「영양제 추천」 「남성 영양제」 「여에스더 유산균」 '
  '같은 새 단어가 그 자리를 채웠습니다. 뿌리가 <b>「칼륨 영양제」·「붓기 영양제」 확장검색</b>이라 '
  '「영양제」가 붙은 모든 검색으로 계속 번집니다. 「영양제」는 우리 키워드라 제외할 수 없습니다.</p>'
  '<p style="margin-top:8px">오늘도 25개를 추가했지만(붓기 146 · 칼륨 75) 이건 시간벌기입니다. '
  '<b>검색 파트너를 끄거나 확장검색 3개를 중지하지 않으면 하루 3만원씩 계속 나갑니다.</b></p>'
  '</div></div>\n')
...
w('<section><div class="sec-h"><h2>광고 켜기 전 기준선</h2>'
  '<span class="sec-n">GA4 · 2026-09-09 ~ 09-26</span></div>')
...
# ── 5. 제외 키워드 ──
w('<section><div class="sec-h"><h2>제외 키워드 146+개</h2>'
  '<span class="sec-n">캠페인 레벨 · 붓기 146 · 칼륨 75 · 브랜드 30</span></div>')
